...
/* [uid], 
  [President/weisser], 
  [Vice President/dklein],
  [Treasurer/sameerab],
  [Cookie Social Chair/jmhardin],
  [Friday Social Chair/kabadi],
  [Friday Social Chair/kjburns],
  [Friday Social Chair/rlynch],
  [Friday Social Chair/wburton],
  [Faculty Breakfast Chair/dklein],
  [Athletics Chair/gdooley],
  [Well-Being Officer/nancyagg] */
...
function getSQL(VoteConfig $conf, MySQLConfig $mysqlconf, ProgramState $state) {
  ...
  if ($state->sqlgetconnect) {
    ...
    if ($eligibility) {
        if (!(isset($state->raw_positions))) {
          $positions = $mysqli->prepare("SELECT * FROM " . $conf->positionstable . " ORDER BY precedence ASC");
          if (!$positions) {
            $state->sqlgetpossuccess = false;
          } else {
            $state->sqlgetpossuccess = $positions->execute();
          }
          if ($state->sqlgetpossuccess) {
            $x = $positions->get_result();
            $state->raw_positions = $x->fetch_all(); 
          } else {
            $state->sqlgetposmessage = $mysqli->error;
          }
        }
        if (!(isset($state->raw_candidates))) {
          $candidates = $mysqli->prepare("SELECT * FROM " . $conf->candidatetable . " ORDER BY position, uid ASC");
          if (!$candidates) {
          ...
          $state->raw_candidates = $x->fetch_all();
          } else {
