...
function voteDump(DOMDocument $xml, VoteConfig $conf, ProgramState $state) {
  ...
  if ($state->sqlgetadminsuccess) {
    ...
    foreach ($voteheadlist as $row) {
      $xml_votes_head1->appendChild($xml->createElement("th", $row));
    }
    ...
    foreach ($voteslist as $row) {
      ...
      $voterid = array_shift($row);
      $xml_first = $xml->createElement("td",$voterid);
      $xml_first->setAttribute("class","voterid");
      $xml_vote_line->appendChild($xml_first);
      foreach ($row as $col) {
        if($col == 1) {
          $xml_check = $xml->createElement("td","Y");
          $xml_check->setAttribute("class", "hellyes");
        } elseif ($col == 0) {
          $xml_check = $xml->createElement("td", "N");
          $xml_check->setAttribute("class", "hellno");
        } else {
          $xml_check = $xml->createElement("td", $col);
        }
        $xml_vote_line->appendChild($xml_check);
      }
...
function candidateFragment(DOMDocument $xml, $precedence, $uid, $realname, ProgramState $state) {
  $pseudoid = $precedence . $uid;
  $xml_candidateline = $xml->createElement("li", NULL);
  $xml_candidatebox = $xml->createElement("input", NULL);
  $xml_candidatebox->setAttribute("name", $pseudoid);
  $xml_candidatebox->setAttribute("value", $pseudoid);
  $xml_candidatebox->setAttribute("type", "checkbox");
  if ($state->sqlgetusersuccess2) {
  if ($state->hasvoted) {
    if ($state->ballot[$pseudoid]) { 
      $xml_candidatebox->setAttribute("checked", "checked");
      $xml_candidateline->setAttribute("class", "chosen");
    }
  }}
  if (isset($_POST[$pseudoid])) {
    if ($_POST[$pseudoid]) $xml_candidatebox->setAttribute("checked", "checked");
  }
  $xml_candidatelabel = $xml->createElement("label", $realname);
  $xml_candidatelabel->setAttribute("for", $pseudoid);
  $xml_candidateline->appendChild($xml_candidatebox);
  $xml_candidateline->appendChild($xml_candidatelabel);
  return $xml_candidateline;
}

function candidateList(DOMDocument $xml, $title, $precedence, VoteConfig $conf, ProgramState $state) {
  if ($state->sqlgetusersuccess) {
      $candidatelist = $state->candidates;
      $xmlcandidate = $xml->createElement("ul", NULL);
      $xmlcandidate->setAttribute("class", "votelist");
      foreach ($candidatelist as list($position, $uid, $realname)) {
      if($position == $title) {
        $xml_candidatefragment = candidateFragment($xml, $precedence, $uid, $realname, $state);
...
function positionFragment(DOMDocument $xml, $title, $maxnumber, $precedence, VoteConfig $conf, ProgramState $state) {
  $xml_positionbox = $xml->createElement("fieldset", NULL);
  $xml_positionbox->setAttribute("name", $title);
  $xml_positionline = $xml->createElement("legend", $title . " (Pick " . $maxnumber . ")");
  $xml_positionbox->appendChild($xml_positionline);
  $xml_actuallist = candidateList($xml, $title, $precedence, $conf, $state);
  $xml_positionbox->appendChild($xml_actuallist);
  return $xml_positionbox;
}
function positionList(DOMDocument $xml, VoteConfig $conf, ProgramState $state) {
  if ($state->sqlgetpossuccess) {
      $positionlist = $state->raw_positions;
      $xmlposition = $xml->createElement("div", NULL);
      $xmlposition->setAttribute("id", "positionlist");
      foreach ($positionlist as list($title, $maxnumber, $precedence)) {
        $xml_positionfragment = positionFragment($xml, $title, $maxnumber, $precedence, $conf, $state);
      $xmlposition->appendChild($xml_positionfragment);
      }
  } else {
    $xmlposition = $xml->createElement("p", $conf->candidatefetcherror . $state->sqlgetposmessage);
    $xmlposition->setattribute("class", "error");
  }
  return $xmlposition;
}
function initForm(DOMDocument $xml, VoteConfig $conf, DOMElement $xmlcandidate, ProgramState $state) {
  ...
  if ($state->hasvoted) {
    $votestring = "You have already voted. You may change your vote using this form.";
  } else {
    $votestring = "You have not yet voted. Please submit your preferences.";
  }
...
function initBody(DOMDocument $xml, VoteConfig $conf, ProgramState $state) {
  ...
  $xml_logo->setAttribute("class", "logo");
  ...
  if ($eligibility and $conf->electionactive) {
    $xml_position = positionList($xml, $conf, $state);
//    $xml_candidate = candidateList($xml, $conf, $state);
    $xml_body->appendChild(initForm($xml, $conf, $xml_position, $state));
